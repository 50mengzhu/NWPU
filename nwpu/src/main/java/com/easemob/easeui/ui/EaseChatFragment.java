...
public class EaseChatFragment extends EaseBaseFragment implements EMEventListener {
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		// 会话人昵称
		toChatUserNickname = fragmentArgs.getString(EaseConstant.EXTRA_USER_NICKNAME);
	...
	protected void setUpView() {
		...
		if (chatType == EaseConstant.CHATTYPE_SINGLE) { // 单聊
			if (!TextUtils.isEmpty(toChatUserNickname)) {
				titleBar.setTitle(toChatUserNickname);
			} else {
				AvatarAndNicknameService.getUserInfoAndSave(toChatUsername, new GetUserInfoCallback() {

					@Override
					public void done() {
						if (AvatarAndNicknameService.userInfoMap != null
								&& AvatarAndNicknameService.userInfoMap.containsKey(toChatUsername))
							titleBar.setTitle(AvatarAndNicknameService.userInfoMap.get(toChatUsername).nickname);
					}
				});
			}
			// ease_mm_title_remove
			titleBar.setRightImageResource(R.drawable.ease_contact_list_normal);
		} else {
	...
	public void toUserProfile() {
		UserProfileActivity.startThisWithStudentId(getActivity(), toChatUsername);
	}
