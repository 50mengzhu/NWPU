...
public class EaseEmojiconScrollTabBar extends RelativeLayout {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		LayoutInflater.from(context).inflate(
				R.layout.ease_widget_emojicon_tab_bar, this);
	...
	public void addTab(int icon) {
		View tabView = View.inflate(context, R.layout.ease_scroll_tab_item,
				null);
		ImageView imageView = (ImageView) tabView.findViewById(R.id.iv_icon);
		imageView.setImageResource(icon);
		LinearLayout.LayoutParams imgParams = new LinearLayout.LayoutParams(
				DensityUtil.dip2px(context, tabWidth),
	...
	public void selectedTo(int position) {
		...
		for (int i = 0; i < tabList.size(); i++) {
			if (position == i) {
				tabList.get(i).setBackgroundColor(
						getResources().getColor(R.color.emojicon_tab_selected));
			} else {
				tabList.get(i).setBackgroundColor(
						getResources().getColor(R.color.emojicon_tab_nomal));
			}
		}
	}
		...
		if (position < childCount) {
			scrollView.post(new Runnable() {
				...
				public void run() {
					...
					int childX = (int) ViewCompat.getMeasuredState(tabContainer
							.getChildAt(position));
					...
					int childWidth = (int) tabContainer.getChildAt(position)
							.getWidth();
	...
	public void setTabBarItemClickListener(
			EaseScrollTabBarItemClickListener itemClickListener) {
