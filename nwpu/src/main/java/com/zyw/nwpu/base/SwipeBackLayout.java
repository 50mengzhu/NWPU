...
public class SwipeBackLayout extends FrameLayout {
	...
	public void setEnableSwipe(boolean is) {
		isEnableSwipe = is;
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (moveX - downX > mTouchSlop && Math.abs((int) ev.getRawY() - downY) < mTouchSlop * 1.5) {
				return true;
			}
			break;
		}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			tempX = moveX;
			if (moveX - downX > mTouchSlop && Math.abs((int) event.getRawY() - downY) < mTouchSlop * 1.5) {
				isSilding = true;
			}
	...
	private void scrollRight() {
		...
		mScroller.startScroll(mContentView.getScrollX(), 0, -delta + 1, 0, Math.abs(delta / 2));
		//
